...
class Dao_Node_Site extends Cl_Dao_Node
{
    ...
    public function afterInsertNode($data, $row)
    {
       ...
       '$inc' => 
    	        array(
    			    ...
    			    'counter.k'=> 
    	                $this->karmaDelta($row['u'],$karmaKey, 'new_story', $row['u']),
    	));
    ...
    public function afterUpdateNode($where, $data, $currentRow)
    {
        ...
        // This only happens if editor/admin approves a story
        // In this case, we should grant OP with a karma which is 
        // the same as OP voting up the story 
        if ($data['$set']['_cl_step'] == 'status' && $this->hotnessRanking)
        {
            ...
            )
        	{
        	    ...
        	    //and the story point & user karma increases
        	    $relationData = array(
        	        'o' => $currentRow,
        	        's' => $data['_u'],
        	        'r' => array('rt' => 1)
        	...
        	)
        	{
        	    ...
        	    //this way, both the relationship exists
        	    //and the story point & user karma decreases accordingly
        	    $relationData = array(
        	        'o' => $currentRow,
        	        's' => $data['_u'],
        	        'r' => array('rt' => 4)
    ...
    public function updateUserKarmaAndNodePoint($actor, $action, $node, $subjectUser = array())
    {
        ...
        //2. update story's point and other counters if neccessary
        	...
        	{
                $storyUpdate['$set'] = array('status' => 'approved','ats'=>time());   

                /*
                 * Tang karma cua nguoi upload khi story do tu vote qua new
                 */
               ...
               '$inc' => 
            	        array(
            	            'counter.p'=> 1,
            			    'counter.k'=> 
            	                $this->karmaDelta($node['u'],$karmaKey, 'new_story', $node['u']),
            	));
            	
            	Dao_User::getInstance()->update($where, $update);
                /**
                 * status queued => approved => story change vote => new 
                 */

            	//Update approved timestamp
            	
            	Dao_Node_Story::getInstance()->initNewOrVoteCacheList('queued',$node['type']);
				Dao_Node_Story::getInstance()->initNewOrVoteCacheList('approved',$node['type']);

            	$typeNode = type_slug($node['type']);
                
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('new',1,10);
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('vote',1,10);
                
                //Delete cahce of widget story
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('widget/new',1,2);
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('widget/vote',1,2);
                
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('new'.$typeNode,1,10);
                Dao_Node_Story::getInstance()-> deleteStaticCacheOfType('vote'.$typeNode,1,10);
        	}
        	elseif ($node['status'] == 'approved' && $newPoint < get_conf($queueThreshold, 10))
        	{
        		$storyUpdate['$set'] = array('status' => 'approved');
        	}
        	
        	$this->update(array('id' => $node['id']), $storyUpdate);
    ...
    public function afterInsertRelation($data, $newRelations, $node)
    {
        ...
        if ($data['r']['rt'] == 1 )//vote up
        {
            if ($this->hotnessRanking)
                $this->updateUserKarmaAndNodePoint($actor, 'story_voted_up', $node);
        ...
        {
            if ($this->hotnessRanking)
                $this->updateUserKarmaAndNodePoint($actor, 'story_voted_down', $node);
        ...
        }elseif ($data['r']['rt'] == 3 )//spam
        {
            ...
            if(isset($spam) && $spam >= get_conf('number_spam',2)){
                ...
                $storyUpdate = array('$inc' => array('counter.spam' => 1));
            }
        ...
        {
            //if voted up => increase 1 more vote
            $storyUpdate = array('$inc' => array('counter.p1_vote' => 1));
            $r = $this->update(array('id' => $node['id']), $storyUpdate);
            if($r['success'])
                $this->removeCahceAndCahceRedis($data,$node);
        }
        ...
        {
            //if voted up => increase 1 more vote
            $storyUpdate = array('$inc' => array('counter.p2_vote' => 1));
            $r = $this->update(array('id' => $node['id']), $storyUpdate);
            if($r['success'])
                $this->removeCahceAndCahceRedis($data,$node);
        }
     ...
     *     
     * A story when moved queued to new will be removed from 'queued'
     * A story when moved from new to hot , can it appear in both??? TODO
     * 
     * 
     * @Note: 
     * Note the distinguish between user's karma and a node's point.
     * Story's point only depends on karma of users who interact with story (such as post, comment, vote, flag...)
     * 
    ...
    public function calculateKarmaDelta($actorKarma, $action, $karmaKey)
    {
    	...
    	switch ($action){
    		...
    		// for story being posted later
    		case 'new_story':
    			$karmaDelta = 5;
    			break;
			//applicable to $subjectUser;
    		case 'del_story':   //when user node is deleted.
    			$karmaDelta = -1 * $this->calculateKarmaDelta($actorKarma, 'new_story') - 1;
    			break;
			...
			case 'story_voted_down':
				$karmaDelta = -1 * $this->calculateKarmaDelta($actorKarma, 'story_voted_up',$karmaKey);
				break;
    ...
    {
    	//what about when A,B,C voted a story
    	//Then the story is a bad story, admin voted down
    ...
    public function pointDelta($actor, $action, $node)
    {
    	...
    	switch($action){
    		case 'story_voted_up':
    			...
    			break;
    		case 'story_voted_down':
    			$point = -1 * $this->pointDelta($actor, 'story_voted_up', $node);
    			break;
    ...
    public function removeStaticCache($row)
    {
        ...
        $filename = $dir . node_link('story', $row);
     ...
     * 		+ remove top_redis:story_new, .....
     * 		+ removed cache story in category vote
     */
    public function removeCahceAndCahceRedis($data,$node){
        ...
        Dao_Node_Story::getInstance()->initNewOrVoteCacheList($data['o']['status'],0);
        Dao_Node_Story::getInstance()->initNewOrVoteCacheList($data['o']['status'],$data['o']['type']);
    }
}
